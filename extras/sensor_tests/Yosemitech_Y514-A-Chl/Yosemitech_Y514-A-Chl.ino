...
byte startMeasurementW[] = {modbusAddress, 0x10, 0x1C, 0x00, 0x00,
                            0x00,          0x00, 0x00, 0x00};
...
void setup() {
    ...
    insertCRC(startMeasurementR,
              sizeof(startMeasurementR) / sizeof(startMeasurementR[0]));
    insertCRC(altStartMeasurementR,
              sizeof(altStartMeasurementR) / sizeof(altStartMeasurementR[0]));
    insertCRC(startMeasurementW,
              sizeof(startMeasurementW) / sizeof(startMeasurementW[0]));
    ...
    insertCRC(activateBrush, sizeof(activateBrush) / sizeof(activateBrush[0]));

    // Start up note
    Serial.println("Yosemitech Y514-A Self-Cleaning Chlorophyll Sensor");

    // Allow the sensor and converter to warm up
    Serial.println("Waiting for sensor to warm up.");
    // TODO:  Figure out how long this *really* needs to be
    delay(5000);
    ...
    if (modbusSerial.available() > 0) {
        ...
        if (bytesRead >= 18) {
            ...
            int j = 0;
            for (int i = 4; i < 16; i++) {
                sn_arr[j] = responseBuffer[i];  // converts from "byte" or "byte" type
                                                // to "char" type
                j++;
            }
            SN = String(sn_arr);
    ...
    driverEnable();
    modbusSerial.write(startMeasurementR,
                       sizeof(startMeasurementR) / sizeof(startMeasurementR[0]));
    modbusSerial.flush();
    // Print the raw send (for debugging)
    Serial.print("Raw Start Measurement Request: ");
    printFrameHex(startMeasurementR,
                  sizeof(startMeasurementR) / sizeof(startMeasurementR[0]), &Serial);
...
void loop() {
    ...
    // TODO: test how long this *really* needs to be
    delay(2000);
}
