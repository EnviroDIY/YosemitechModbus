...
int State9 = LOW;
int incomingByte =
    0;  // for incoming serial data. Anthony Note: where to store the bytes read
// Anthony Note: "unsigned char" datatype is equivalent to "byte".
// https://oscarliang.com/arduino-difference-byte-uint8-t-unsigned-cha/
unsigned char buffer[13];   // Anthony Note: Allocate some space for the Bytes, as
                            // 13-element array of bytes
unsigned char command[16];  // Anthony Note: Allocate some space for the Bytes, as
                            // 16-element array of bytes
float Temperature, Conductivity;
byte  error;
// float Temperature, Conductivity, SN; //Beth note: trying to print serial number in
// header
...
// //This is from the DO sketch, and Cond always = 0 but T works
unsigned char getTempandCond[8] = {
    0x01, 0x03, 0x26, 0x00, 0x00,
    0x05, 0x8E, 0x81};  // This is from the Conductivity instruction manual, ovf
                        // "overflow as floats" error occurs
...
void setup() {
    ...
    // delay(12);
    // digitalWrite(12, LOW);
    delay(10000);  // Beth note: user manual says to wait 10 seconds before
                   // conductivity, then to use as average
    ...
    // Serial.print("Sesnor SN "); Serial.println(SN); //Beth note: trying to print
...
void loop() {
    ...
    State9 = !State8;  // Anthony Note: Assign State9 to be NOT State8 (the opposite of
                       // State8)
    ...
    if (Serial1.available() > 0) {
        ...
        // say what you got:
        if (incomingByte == 15) {
            Temperature = Rev_float(
                buffer, 3);  // Beth: does this mean Rev_float location at byte 3?
            Conductivity = Rev_float(
                buffer, 7);  // Beth note: is this the error in reading conductivity?
